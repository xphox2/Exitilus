...
export class GameEngine {
  ...
  private async showTitle(): Promise<void> {
    ...
    // Animate: show copyright, wait, replace with credit
  ...
  private async animateCredit(row: number, col: number, len: number, color: string): Promise<void> {
    await new Promise(r => setTimeout(r, 2000));
    const pos = `\x1B[${row};${col}H`;
    this.session.write(pos + ' '.repeat(len));
    await new Promise(r => setTimeout(r, 300));
    this.session.write(pos + color + 'Ported by Xphox in 2026' + ANSI.RESET);
  }
  ...
  private async entryMenu(): Promise<void> {
    while (true) {
      ...
      this.session.writeln(`                       ${ANSI.CYAN}Ported by Xphox in 2026${ANSI.RESET}`);
      this.session.writeln('');
